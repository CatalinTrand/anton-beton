...
const postableImage = (image: { uri: string; path: string }) => {
  try {
...
const antiDuplication = (items: Record<string, string>[], uniqueKey: string | number) => {
  const uniq = {};
...
const groupObjectByKey = (objectData: Record<string, string>[], groupBy: string | number) => {
...
const makeSectionListFormat = (
  dataArrayOfObjects: Record<string, string>[],
...
const orderTimeCounter = (startTime: string, numberOfMinutes: number, endTime = null) => {
